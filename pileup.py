...
def pileup_calc(table, f, exptime):
	...
	for i, count_error_point in enumerate(count_error):
		if count_error_point == 0.0:
			pileup_error[i] = count_error_point
		else:
			intersection = lambda x: nongrating_equation(x) - count_error_point
...
def pileup_correction_contam(observationID, repro_wd, erange, tbin, fileName):
	...
	for i in range(3):
		#Open the lightcurve.
		f = fits.open(f'{repro_wd}/{observationID}_contam{i}_{erange[0]}-{erange[1]}keV_lc{tbin}.fits')
		table = Table(f[1].data)

		#Open the event files to get the exposure time of the observation.
		f_evt = fits.open(f'{repro_wd}/acisf{observationID}_contam{i}_{erange[0]}-{erange[1]}keV_evt.fits')
		exptime = float(f_evt[1].header['EXPTIME'])
		...
		# Assemble final file with original PRIMARY and GTI extensions
		hdul = fits.HDUList([f[0], hdu1, f[2]])
		hdul.writeto(f'{repro_wd}/{observationID}_contam{i}_{erange[0]}-{erange[1]}keV_lc{tbin}_pileup.fits', overwrite=True)
