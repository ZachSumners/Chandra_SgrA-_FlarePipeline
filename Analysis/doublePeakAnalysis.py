import os
import numpy as np
from astropy.io import fits
import pandas as pd
import matplotlib.pyplot as plt
from sklearn.decomposition import PCA
from scipy.optimize import curve_fit

df = pd.read_csv('flare_properties.csv')
flare_numbers = df['flare_number']
flare_ids = df['obs_id']
max_rate = df['rate_max']
mjd_starts = df['start_mjd']
mjd_ends = df['end_mjd']
j = 0

quiescent_df = pd.read_csv('quiescent_rates.csv')
all_plots = []

singlepeak_ids = [11843, 10556, 13838, 13839, 13842, 13854, 13852, 13857, 14463, 14466, 15041, 15042, 15045, 16963, 16508, 18731, 20446, 21454, 22230, 23666, 23665, 23739, 28231, 3393, 4684, 6363]
doublepeak_ids = [10556, 13843, 13847, 13851, 13849, 14462, 14468, 14944, 15043, 15042, 15570, 16966, 18732, 20041, 20346, 21454, 21456, 25297, 26760, 3393, 5953, 1561]

for i, flare in enumerate(flare_ids):

    if flare not in doublepeak_ids:
        continue

    flare_number = flare_numbers[i]
    if (flare == 10556 and flare_number == 1) or (flare == 10556 and flare_number == 2) or (flare == 10556 and flare_number == 4) or (flare == 14462 and flare_number == 1) or (flare == 14468 and flare_number == 2) or (flare == 15042 and flare_number == 1) or (flare == 21454 and flare_number == 1):
        continue
    
    flare = str(flare)
    if flare == '1561':
        lc = f"./1561/1561_000/repro/01561_sgra_2-8keV_lc300_pileup.fits"
    elif len(flare) < 5:
        flare_5digit = str(flare).zfill(5)
        lc = f"./{flare}/repro/{flare_5digit}_sgra_2-8keV_lc300_pileup.fits"
    else:
        lc = f"./{flare}/repro/{flare}_sgra_2-8keV_lc300_pileup.fits"
    f = fits.open(lc)
    data = f[1].data 

    chandra_time = data['TIME']
    time = 50814.0 + (chandra_time / 86400.0)
    rate = data['RATE_PILEUP']
    mjd_start_flare = mjd_starts[i]
    mjd_end_flare = mjd_ends[i]
    max_rate_reported = max_rate[i]


    mjd_start = 51844.0
    sec_per_day = 86400

    # Identify points in the flare window
    flare_mask = (time >= mjd_start_flare) & (time <= mjd_end_flare)
    flare_times = time[flare_mask]
    flare_rates = rate[flare_mask]


    max_idx = np.where(np.around(flare_rates, decimals = 4) == max_rate_reported)
    
    if len(flare_times) == 0:
        print(f"No lightcurve data inside flare {i+1}")
        continue

    # Find brightest datapoint
    peak_idx = np.argmax(flare_rates)
    peak_time = flare_times[peak_idx]
    peak_rate = flare_rates[peak_idx]

    # Extract ±4200s around the peak
    window_mask = (time >= peak_time - 4200/86400) & (time <= peak_time + 4200/86400)
    
    flare_window = rate[window_mask]#/peak_rate

    peak_idx_flare_window = np.where(flare_window == peak_rate)

    if len(flare_window) != 29:
        if len(peak_idx_flare_window) == 1:
            flare_window = np.concatenate([np.zeros(14 - peak_idx_flare_window[0][0]), flare_window])
            flare_window = np.concatenate([flare_window, np.zeros(29 - len(flare_window))])
        else:
            peak_rate_single_flare_idx = np.min(np.abs(14 - peak_idx_flare_window))[0]
            flare_window = np.concatenate([np.zeros(14 - peak_rate_single_flare_idx), flare_window])
            flare_window = np.concatenate([flare_window, np.zeros(29 - len(flare_window))])

    quiescent_rate = quiescent_df.loc[quiescent_df['obs_id'] == int(flare), 'quiescent_rate'].values
    flare_window -= quiescent_rate[0]/1000

    flare_window = flare_window/(peak_rate-quiescent_rate[0]/1000)
    #flare_window = (flare_window - np.min(flare_window)) / (np.max(flare_window) - np.min(flare_window))
    
    all_plots.append(flare_window)

    flare_window = np.where(flare_window > 1, 0, flare_window)
    plt.plot(np.arange(-4200, 4201, 300), flare_window, c='black', linewidth=1, alpha=0.3)


X = np.vstack(all_plots)
X_centered = X - np.mean(X, axis=0)
pca = PCA(n_components=1)
pca.fit(X_centered)

first_basis = pca.components_[0]
proj = X_centered @ first_basis
recon = np.outer(proj, first_basis)
recon_mean = np.mean(recon, axis=0) + np.mean(X, axis=0)


average_flare = np.mean(X, axis=0)

# Define the model
def exp_decay(t, tau):
    return np.exp(-t / tau)
def exp_decay2(t, tau):
    return np.exp(t / tau)

# Fit the model
popt, pcov = curve_fit(exp_decay, np.arange(1, len(average_flare[14:])+1, 1), average_flare[14:])  # initial guesses
tau_fit = popt

popt2, pcov2 = curve_fit(exp_decay2, np.arange(-len(average_flare[:15]), 0, 1), average_flare[:15])  # initial guesses
tau_fit2 = popt2


def uncert_bootstrap(flare_array):
    sample_indices = np.random.choice(flare_array.shape[0], size=63, replace=True)
    flare_array_sampled = flare_array[sample_indices]   

print(recon_mean.shape)
plt.plot(np.arange(-4200, 4201, 300), recon_mean, c='red', linewidth=3, label='PCA')
#plt.plot(np.arange(-4200, 4201, 300), np.mean(X, axis=0), c='blue', linewidth=3, label='Mean')
plt.plot(np.arange(0, 4201, 300), exp_decay(np.arange(0, 4201, 300), tau_fit*300), c='orange', linewidth=3, label=f'Fit to mean, decay factor = {tau_fit[0]*300:.2f}')
plt.plot(np.arange(-4200, 1, 300), exp_decay(np.arange(-4200, 1, 300), -tau_fit2*300), c='orange', linewidth=3, label=f'Fit to mean, rise factor = {tau_fit2[0]*300:.2f}')

#plt.ylim(-0.05, 1.05)
plt.legend()
plt.xlabel('Time (seconds)')
plt.ylabel('Normalized Flux')
plt.title('Characteristic Flare Shape')
plt.show()